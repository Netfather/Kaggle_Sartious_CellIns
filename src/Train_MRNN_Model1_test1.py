...
df_images = total_df.groupby(["id", "cell_type"]).agg({'annotation': 'count'}).sort_values("annotation", ascending=False).reset_index()
...
df_images_train, df_images_val = train_test_split(df_images, stratify=df_images['cell_type'],
                                                  test_size=hyper_parameter_group["PCT_IMAGES_VALIDATION"],
                                                  random_state= hyper_parameter_group["seed"] )

df_train = total_df[total_df['id'].isin(df_images_train['id'])]
df_val = total_df[total_df['id'].isin(df_images_val['id'])]

# total_df.loc[total_df['id'].isin(df_images_train['id']),"test1_fold"] = 1
# total_df.loc[total_df['id'].isin(df_images_val['id']),"test1_fold"] = 0
#
# total_df.to_csv("./train_test1_only_use.csv",index = False)
...
for epoch in range(1, hyper_parameter_group["total_epoch"] + 1):
    ...
    torch.save(model.state_dict(),hyper_parameter_group["out_dir"] + '/checkpoint/model_{}.pth'.format(
                                    epoch))
    prefix = "[Epoch {} / {}]".format(epoch,hyper_parameter_group["total_epoch"])
    stdout(prefix)
    stdout(f"{prefix} Train mask-only loss: {train_loss_mask:7.3f}, classifier loss {train_loss_classifier:7.3f}")
    stdout(f"{prefix} Val mask-only loss  : {val_loss_mask:7.3f}, classifier loss {val_loss_classifier:7.3f}")
    stdout(prefix)
    stdout(f"{prefix} Train loss: {train_loss:7.3f}. Val loss: {val_loss:7.3f} [{elapsed:.0f} secs]")
    stdout(prefix)