...
#         2. 使用LiveCell论文中的全部结构，除了 FPN in feature 会对显存造成显著影响而未加入
#         3. 使用 resx101 的backbone，没有做任何修改，学习率设定为 0.0001
...
#         7. 2021年12月24日更新： 目前统一一下： 所有的 res50 结构 统一使用 "default" 设置
#                                              所有的 resx101 结果 统一使用 "livecell" 设置
#
#    结论：
# test40: 1. 在 LiveCell上预训练
#         2. 使用LiveCell论文中的全部结构，除了 FPN in feature 会对显存造成显著影响而未加入
#         3. 使用 res50 的backbone，没有做任何修改，学习率设定为 5e-3       
#         4. 为了加快迭代速度，将总 iteration 降低为 3500, multi-step-lr 做相应修改 
...
# test42: 1. 初步怀疑 cascade 无法在livecell中得到任何训练，因此 预训练权重修改为 官方的  cascade 模板
#         2. 使用LiveCell论文中的全部结构，除了 FPN in feature 会对显存造成显著影响而未加入
#         3. 使用 res50 的backbone，没有做任何修改，学习率设定为 5e-3       
#         4. 为了加快迭代速度，将总 iteration 降低为 3500, multi-step-lr 做相应修改 
#         5. 使用2021的 split 划分
...
def setup(args,fold_id):
    ...
    cfg.OUTPUT_DIR  ="/storage/Kaggle_Cell_Segmentation/model/MaskRNN/test42"
    ...
    # cfg.MODEL.WEIGHTS = "/storage/Kaggle_Cell_Segmentation/model/model_final_Cascade.pkl"
    
    cfg.MODEL.WEIGHTS = '/storage/Kaggle_Cell_Segmentation/model/MaskRNN/Res50Pretrained_Version6/model_best_fold1.pth'   # Let training initialize from the pretrained model
