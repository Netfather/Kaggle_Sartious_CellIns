...
def Train_Per_Fold(fold_id,args):
    ...
    meta_iteration = len(DatasetCatalog.get('sartorius_train{}'.format(fold_id))) // cfg.SOLVER.IMS_PER_BATCH
    ...
    cfg.SOLVER.CHECKPOINT_PERIOD = meta_iteration * 51 # 不需要保存每个 epoch  相应信息保存在best_checkpoint 中
    ...
    # 测试指标
    cfg.TEST.EVAL_PERIOD = meta_iteration  # Once per epoch
