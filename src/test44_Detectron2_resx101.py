...
#
#    结论：
# test36: 1. 在 LiveCell上预训练
#         2. 使用LiveCell论文中的全部结构，除了 FPN in feature 会对显存造成显著影响而未加入
#         3. 使用 resx101 的backbone，没有做任何修改，学习率设定为 5e-3
#             2021年12月23日更新： loss会在一开始疯狂下降，然后升高， 可能 0.001的学习率不适合，予以修正
#         4. 为了加快迭代速度，将总 iteration 降低为 3500, multi-step-lr 做相应修改 
#         5. 使用3407的 split 划分，本次使用的是 3407 种子 而且划分依据不再是细胞类型，而是 每张图中 annotation的数量
#         6. 经过测试 bie 版本的表现效果并不好， 修正为 官方推荐的版本
#         7. 2021年12月23日更新： 修复了官方的 coco_eva 的问题，现在可以正确使用了 
#            2021年12月24日更新： 现在计算的几乎百分百就是 local_cv的内容了，绝对正确   
#         8. 2021年12月24日更新： 目前统一一下： 所有的 res50 结构 统一使用 "default" 设置
...
#
#    结论：
# test41: 1. 在 LiveCell上预训练
#         2. 使用LiveCell论文中的全部结构，除了 FPN in feature 会对显存造成显著影响而未加入
#         3. 使用 resx101 的backbone，没有做任何修改，学习率设定为 5e-3
#         4. 为了加快迭代速度，将总 iteration 降低为 3500, multi-step-lr 做相应修改 
#         5. 使用3407的 split 划分，本次使用的是 3407 种子 划分依据是细胞类型
#         7. 2021年12月23日更新： 修复了官方的 coco_eva 的问题，现在可以正确使用了 
#            2021年12月24日更新： 现在计算的几乎百分百就是 local_cv的内容了，绝对正确   
#         8. 2021年12月24日更新： 目前统一一下： 所有的 res50 结构 统一使用 "default" 设置
#                                              所有的 resx101 结果 统一使用 "livecell" 设置
#
#    结论： 
# test43: 1. 在 LiveCell上预训练
#         2. 使用LiveCell论文中的全部结构，除了 FPN in feature 会对显存造成显著影响而未加入
#         3. 使用 resx101 的backbone，没有做任何修改，学习率设定为 1e-3
#         4. 为了加快迭代速度，将总 iteration 降低为 5000, multi-step-lr 做相应修改 
#         5. 使用3407的 split 划分，本次使用的是 3407 种子 划分依据是细胞类型
#         7. 2021年12月23日更新： 修复了官方的 coco_eva 的问题，现在可以正确使用了 
#            2021年12月24日更新： 现在计算的几乎百分百就是 local_cv的内容了，绝对正确   
#         8. 2021年12月24日更新： 目前统一一下： 所有的 res50 结构 统一使用 "default" 设置
#                                              所有的 resx101 结果 统一使用 "livecell" 设置
#         9. 已经确认  deform V1 的结果表现很糟糕，予以放弃
#
#    结论：
...
def setup(args,fold_id):
    ...
    cfg.OUTPUT_DIR  ="/storage/Kaggle_Cell_Segmentation/model/MaskRNN/test44"
    os.makedirs(cfg.OUTPUT_DIR, exist_ok=True)
    cfg.INPUT.MASK_FORMAT='bitmask'
    register_coco_instances('sartorius_train{}'.format(fold_id),{}, '../data/1997_split_train_fold{}.json'.format(fold_id), dataDir)
    register_coco_instances('sartorius_val{}'.format(fold_id),{},'../data/1997_split_val_fold{}.json'.format(fold_id), dataDir)
