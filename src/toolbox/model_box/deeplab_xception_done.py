...
# 此函数用于深度可分离卷积中，为了将空洞卷积的卷积核和输入的x大小匹配，手动进行padding
...
class SeparableConv2d_same(nn.Module):
    def __init__(self, inplanes, planes, kernel_size=3, stride=1, dilation=1, bias=False):
        ...
        self.conv1 = nn.Conv2d(inplanes, inplanes, kernel_size, stride, 0, dilation,
                               groups=inplanes, bias=bias) # 先对尺寸 也就是这里通过卷积和尺寸来进行尺寸的调节。
        self.pointwise = nn.Conv2d(inplanes, planes, 1, 1, 0, 1, 1, bias=bias) # 再对通道
...
class Block(nn.Module):
    def __init__(self, inplanes, planes, reps, stride=1, dilation=1, start_with_relu=True, grow_first=True, is_last=False):
        ...
        :param inplanes: 输入通道
        :param planes: 输出通道
        :param reps: 在这个残差模块中 有几个是一模一样的卷积层
        :param stride:
        :param dilation:
        :param start_with_relu: 是否是开始卷积层，开始卷积层是不带有relu
        :param grow_first:
        :param is_last: 是否是最后一个卷积层
        '''
        super(Block, self).__init__()

        # 实现skip操作，确认主干道是否存在下采样，如果是则skip中存在下采样的卷积
...
class ASPP_module(nn.Module):
    def __init__(self, inplanes, planes, os = 16):
        '''
        This ASPP is using in deeplabv3+. The input tensors goes throw 5 parallel pathes.
        :param inplanes: The features channels of your inputs
        :param planes: The features channels of evry pathes.
        :param os: The output stride. Besides on your needings
        '''
        ...
        if os == 16:
            ...
            raise ("ASPP output stride settings error!")
        ...
        self.global_avg_pool = nn.Sequential(nn.AdaptiveAvgPool2d((1, 1)),
                                             # 这里的2048是一个预先定义好的输出通道，如果要修改就需要在这里修改
                                             nn.Conv2d(2048, planes, 1, stride=1, bias=False),
                                             nn.BatchNorm2d(planes),
                                             nn.ReLU())
        self.conv1 = nn.Conv2d(1280, 256, 1, bias=False) # 这里默认了 经过ASPP模块之后，通道数为1280 planes*5
