...
# -------------------------------------------------------------------------------
# Name:         FCN
# Description:  此文件为FCN网络构建第二步 实现FCN的模型
...
# 使用这个函数可以构建一个满足插值大小的np.zeros的权重
...
# 手动加载checkpoint
pretrained_net.load_state_dict(torch.load("./vgg16_bn-6c64b313.pth"))
...
class FCN(nn.Module):
    ...
    def forward(self, x):
        ...
        if __name__ == '__main__': print(s5.shape)

        scores1 = self.scores1(s5)
        s5 = self.upsample_2x_1(s5)
        add1 = s5 + s4

        if __name__ == '__main__': print(add1.shape)
        ...
        if __name__ == '__main__': print(add2.shape)
