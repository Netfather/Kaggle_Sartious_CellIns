...
#           不使用过拟合，学习率调低1/5查看结果
# 2021年6月8日   V7. 4头注意力，极强正则化，学习率调低，查看结果
# 2021年6月10日  V9. 原本想修改 注意力的内部机制，但是发现无法修改，因此将注意力调整为8头 加入极强正则化 0.5查看结果
...
class MLA(nn.Module):
    def __init__(self,w:int,h:int,patch_size:int,depth = 4):
        ...
        self.blocks = nn.Sequential(*[
            Block(
                dim=1024, num_heads=8, mlp_ratio=4., qkv_bias=False, qk_scale=None,
                drop=0.5, attn_drop=0.5, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer)
            for i in range(depth)])
