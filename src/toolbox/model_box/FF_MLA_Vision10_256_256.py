...
# #                   同时不删去最底层，查看结果。
# 2021年10月20日 V？？ 特化版本  将原有针对  592 592 的原模型 改为 256 * 256 同时增大中间Tr层的深度
...
class PatchEmbed(nn.Module):
    ...
    """
    def __init__(self, img_size=32, patch_size=32, in_chans=1, embed_dim= 384, norm_layer=None):
        super().__init__()
        ...
        # 如下为 proj的两种可能方式 一种为linear 适用于小分辨率 一种为 conv2d 适用于较大分辨率
        # self.proj =  nn.Linear(img_size[0]*img_size[1], embed_dim) # 2021年10月20日 对于 256 256 的输入 为了降低参数 没有必要进行 proj
    ...
    def forward(self, x):
        ...
        # 如下三行对应的是原版的映射方法 使用conv卷积
        # x = self.proj(x)
        # print(x.shape)
        # x =  x.flatten(2).transpose(1, 2)

        # 如下使用的是 flatten方法 为了减少运算量 直接通过linear进行
        x = x.flatten(2)
        # x = self.proj(x)  # 2021年10月20日 对于 256 256 的输入 为了降低参数 没有必要进行 proj
        x = self.norm(x)
        return x
...
class MLA(nn.Module):
    def __init__(self,w:int,  h:int,   patch_size, num_heads = 6,  depth = 4,  emb_dim = 384 ,drop_rate = 0.1, drop_path_rate = 0.4):
        super(MLA, self).__init__()
        # 这里的 w h 直接给定最后一层的 特征图大小
        self.W = w
        self.H = h
        ...
        self.blocks = nn.Sequential(*[
            Block(
                dim= emb_dim , num_heads= num_heads, mlp_ratio= 0.5, qkv_bias=True, qk_scale=None,
                drop= drop_rate, attn_drop=0., drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer)
...
class UNet(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=True,transformer_drop_rate = 0.2,drop_path_rate = 0.4):
        ...
        self.mla = MLA(32,32,(32,32), num_heads= 8 ,depth= 6, emb_dim= 1024 ,drop_rate= transformer_drop_rate , drop_path_rate = drop_path_rate)
