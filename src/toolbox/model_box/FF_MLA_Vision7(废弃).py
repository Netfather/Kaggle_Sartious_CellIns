...
class PatchEmbed(nn.Module):
    ...
    """
    def __init__(self, img_size=32, patch_size=32, in_chans=1, embed_dim=1024, norm_layer=None):
        super().__init__()
        ...
        # 2021年6月21日 修正： 由于 nn.Liner无法进行自适应的分配分辨率，因此弃用 nn.Liner 改用 自适应pooling
        # self.proj1 = nn.AdaptiveAvgPool2d(output_size=patch_size)
...
class MLA(nn.Module):
    def __init__(self,w:int,h:int,patch_size:int,depth = 1,emb_dim = 1024,drop_rate = 0.1):
    ...
    def forward(self,x):
        ...
        # 2021年6月21日 修正 这里SoftMax是竞争关系 修正为 sigmoid查看结果
        # 2021年7月2日修正  修改了最后出来的自注意力的处理方式 原来是Sigmoid方式
        test_channel = torch.softmax(test,dim= 1)
        test_spatial = torch.softmax(test,dim = -1) * test_channel



        # 将test中  每个维度都还原为 32 * 32
        B,C,_ = test.shape
        test_spatial = test_spatial.reshape(shape = [B,C,32,32])

        #new_x4 = F.interpolate(input=torch.unsqueeze(test_spatial[:, 3, :, :], dim=1), size=(x[3].shape)[2:4],mode= "bilinear",align_corners= True)
        new_x3 = F.interpolate(input=torch.unsqueeze(test_spatial[:, 2, :, :], dim=1), size=(x[2].shape)[2:4],mode= "bilinear",align_corners= True)
        new_x2 = F.interpolate(input=torch.unsqueeze(test_spatial[:, 1, :, :], dim=1), size=(x[1].shape)[2:4],mode= "bilinear",align_corners= True)
        new_x1 = F.interpolate(input=torch.unsqueeze(test_spatial[:, 0, :, :], dim=1), size=(x[0].shape)[2:4],mode= "bilinear",align_corners= True)
...
class UNet(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=True,transformer_drop_rate = 0.1):
        ...
        self.mla = MLA(148,148,148,depth= 1,emb_dim= 148 * 148,drop_rate= transformer_drop_rate)
