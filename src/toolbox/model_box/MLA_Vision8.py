...
# 2021年6月10日 V8. 版本修复 由于直接使用的 cbam思想 但是还是乘了一遍原数字，这相当于 对V数组进行了平方，这是明显不对的 因此做出修正
...
class MLA(nn.Module):
    ...
    def forward(self,x):
        ...
        # 2021年6月10日 修正  直接用cSE的结果进行处理
        spatial_attention = self.sSE(test)
        channel_attention = self.cSE(test)
        # 对空间维度上的数值重复4次
        spatial_attention = torch.repeat_interleave(spatial_attention,repeats= 4,dim= 1)
        test = spatial_attention * channel_attention
        # print(test.shape)
        # print(test.max())
        # print(test.min())
